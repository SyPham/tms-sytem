...
export class CommentComponent implements OnInit, AfterViewChecked, OnDestroy {
 ...
 public menuItems: MenuItemModel[] ;
  ...
  ngAfterViewChecked() {
    this.marginTop();
    const currentUser = JSON.parse(localStorage.getItem('user')).ID as number;
  ...
  imageBase64CurrentUser() {
    if (JSON.parse(localStorage.getItem('user')).ImageProfile === null) {
      return this.defaultImage();
    } else {
      return this.sanitizer.bypassSecurityTrustResourceUrl(
        'data:image/png;base64, ' + JSON.parse(localStorage.getItem('user')).image);
    }
  }
  defaultImage() {
  ...
  contextMenuOption() {
    const currentUser = JSON.parse(localStorage.getItem('user')).OCLevel as number;
    if (this.pin?.CreatedTaskBy === currentUser || this.pin?.CreatedProjectTaskBy === currentUser) {
      this.menuItems = [
        {
            text: 'Edit',
